...
VertexOut VSMain(VertexIn vin, uint instanceID : SV_InstanceID) {
  ...
  vout.NormalW = mul(vin.Normal, (float3x3)(insData.gInvTransWorld));

  float4 PosW = mul(float4(vin.PosL, 1.0), insData.gWorld);
  vout.PosW = PosW.xyz;
  vout.PosH = mul(PosW, gViewProj);

  MaterialData matData = gMaterialData[insData.gMaterialIndex];
  float4 texC = mul(float4(vin.TexC, 0.0f, 1.0f), insData.gTexTransform);
  vout.TexC = mul(texC, matData.gMatTransform).xy;
  vout.MatIndex = insData.gMaterialIndex;
...
float4 PSMain(VertexOut vout) : SV_Target {
  ...
  MaterialData matData = gMaterialData[vout.MatIndex];
